...
TEAM_CODE  = "oita"
FL_BASE    = "https://www.football-lab.jp"
YAHOO_BASE = "https://soccer.yahoo.co.jp/jleague/category/j2j3ss/game"


def build_fl_url(date_str: str, team: str = TEAM_CODE) -> str:
    """'YYYYMMDD' → Football LAB マッチレポートURL"""
    y, m, d = date_str[:4], date_str[4:6], date_str[6:8]
    return f"{FL_BASE}/{team}/report?year={y}&month={m}&date={d}"


def build_yahoo_url(game_id: str) -> str:
    """'YYYYMMDDXX' → Yahoo スポーツナビ テキスト速報URL（空なら空文字列）"""
    return f"{YAHOO_BASE}/{game_id}/text?gk=250" if game_id else ""
...
def main(url: str, yahoo_url: str = "", output_dir: str = ".") -> None:
    ...
    out = Path(output_dir) / derive_filename(url)
    out.write_text(content, encoding='utf-8')
    print(f"Saved: {out}")


if __name__ == '__main__':
    date_str   = sys.argv[1] if len(sys.argv) > 1 else "20260419"
    game_id    = sys.argv[2] if len(sys.argv) > 2 else ""
    output_dir = sys.argv[3] if len(sys.argv) > 3 else "."
    main(build_fl_url(date_str), build_yahoo_url(game_id), output_dir)
